...
public class CaseProcessorTest {
    ...
    @Test public void testEmitCaseCreatedEvent() {
        // Given
        Case caze = new Case();
        caze.setRgn("E");
        ...
        // Then
        ArgumentCaptor<ResponseManagementEvent> rmeArgumentCaptor = ArgumentCaptor.forClass(ResponseManagementEvent.class);
        verify(rabbitTemplate).convertAndSend(eq("TEST_EXCHANGE"), eq(""), rmeArgumentCaptor.capture());
        assertEquals("TEST_POSTCODE", rmeArgumentCaptor.getValue().getPayload().getCollectionCase().getAddress().getPostcode());
    }
